...
def httpv1_regex_ip_swap(packet, data):
    ...
    Updates the Content-Length header if the body byte length changes
    as a result of IP address substitution.
    ...
    if header_sep in s:
        ...
        if old_body_len != new_body_len:
            content_length_re = re.compile(r'(?i)(Content-Length:\s*)(\d+)')
            new_headers = content_length_re.sub(
                lambda m: m.group(1) + str(new_body_len),
                new_headers
            )
...
def httpv1_rewrite_date(packet, data):
    """
